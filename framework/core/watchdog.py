...
def _kill_the_processes(target, proc_list, func_list):
	...
	if target == "localhost":
		...
		for proc in (proc_list, ):
			utils.stop_remote(target, proc, hasOutput=False)
...
def launch_watchdog(target, timeout_ms, proc_list, func_list):
	global _watchdog_lock, _watchdog_active, _watchdog_state, _watchdog_timer
	_watchdog_lock.acquire()
	if (_watchdog_active == False):
		_watchdog_state = False # we havent killed anything yet
		_watchdog_timer = Timer(timeout_ms / 1000, _kill_the_processes, [target, proc_list, func_list])
