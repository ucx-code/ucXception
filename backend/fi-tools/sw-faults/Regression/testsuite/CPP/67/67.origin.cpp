...
/* Quicksort recursivo simples com pivot no meio */
...
int main()
{
     ...
     //qsort(&begin[1], n, sizeof(int), compare);
     quick_sort(&begin[0], n);
     quick_sort(&end[0], n);
     //qsort(&end[1], n, sizeof(int), compare);
     
     unsigned int conta = 0; unsigned int a = 0;
     for (i = 0; i < n; i++)
     {
          if (begin[i] < end[a]) ++conta;
          else ++a;
     }
     printf("%d\n", conta);
