...
Reordered ifs
Removed queue and started recurse from the end
Buffered output
*/
#include <stdio.h>
#include <stdlib.h>
...
/* Strcat rapido porque nao ta sempre a meter \0 no final */
...
int recurse(int pos, int max, int width)
{
	...
	if (bufferPos == '|')
	{
		if (bufferPosWidth == '/')
		{
			...
			{
				if (recurseSize != 0) strcat2((char *)",", 1);
				sprintf(tmp, "(%d,%d)",  getY(pos, width), getX(pos, width));
				strcat2(tmp);			
				return 1;
			}
...
int main()
{
	...
	{	
		...
		if (!encontrou) strcat2((char *) "No Path!\n", 9);
	} 
