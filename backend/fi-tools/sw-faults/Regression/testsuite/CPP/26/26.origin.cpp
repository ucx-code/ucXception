...
std::vector<int> MSC(std::vector<int> S, std::vector<int> U, int c)
{
	...
	{
		if (U.size() == 0)
			{ std::vector<int> empty; return empty; }
		else { std::vector<int> bad;bad.push_back(-1); return bad; }
	}
	// Find set of maximum cardinality
	int maior =-1; int maiorID=-1;	
	int i;
	...
	{ std::vector<int> bad;bad.push_back(-1); return bad; }
