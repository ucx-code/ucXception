...
int calc(int *c)
{
     ...
     for (i = 0; i < m; i++)
     {
          if (estadoLast[i] == 0) soma+= friendship[i];
     }
...
{
     int i,a;
     //int estado[2000]; 5111421 2800004
     //memset(estado, 0, sizeof(unsigned char) * m);
     //memcpy(estado, preCalculated[begin-1], sizeof(int) * m);   
    /* for (i = 0; i < begin; i++)
     {
          if (c[i] == 0)
               for (a = 0; a < listaC[i][0]; a++)
     ...
     for (i = 0; i < m; i++)
     {
          if ((estadoLast[i] == 0)&&(preCalculated[begin-1][i]==0)) soma+= friendship[i];
     }
     return soma;
}
...
/*void preCalcs()
{
     ...
     for (i = 0; i < n; i++)
     {
          memset(estados, 0, sizeof(int)*200);
            for (a = 0; a < listaC[i][0]; a++)
            {
                              estados[lista[i][0][a]] = 1;
            }
          for (a = 0; a < listaC[i][1]; a++)
          {
                    estados[lista[i][1][a]] = 1;
          }
          for (a = 0; a < m; a++)
               if (estados[a] == 0) preCalc[i] += friendship[i];     
     }
}*/
