...
from framework.core.remote_proc import *
...
def proc_communicate(proc):
	...
	#Byte to string
	return (str(out, 'utf-8'), str(err, 'utf-8'), proc.returncode)
...
def proc_launch_ssh(ssh_config, program, args, stdin=None, stdout=None, stderr=None, background=True):
	...
	if not background:
		p = subprocess.Popen(["/usr/bin/ssh", "-o StrictHostKeyChecking=no", "-o ServerAliveInterval=5", "-o ServerAliveCountMax=6", login_info, cmd_arg],
		...
		return (str(out, 'utf-8'), str(err, 'utf-8'), p.returncode)
		...
		p = subprocess.Popen(["/usr/bin/ssh", "-o StrictHostKeyChecking=no", "-o ServerAliveInterval=5", "-o ServerAliveCountMax=6", login_info, ssh_arg],
							 stdin=None, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False)
		out = p.communicate()
