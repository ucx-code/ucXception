...
def launch_fi(target, fi_path, source_file, preprocess=True, check_extension=True, operator=None, c_parser=True):
	...
	(output, rcode, nothing) = utils.run_anywhere(target, "java", " ".join(flags), None, None, False)
	logger.debug(output)
	logger.debug(rcode)
	...
	#Position zero from tuple
	for file in utils.listdir_anywhere(target, local_dir)[0]:
