...
int unitPropagation()
{
     int i,a,j,k,z; bool tem;
     for (i = 0; i < n; i++)
     {
          for (a = 0; a < unitClauseC[i][1]; a++)
          {
               /* Apaga todas as clausulas que contem i */  
               for (j = 0; j < m; j++)
               {
                    tem = false;
                    for (k = 0; k < 3; k++)
                    {
                         if (choices[j][k] == 0) break;
                         if (choices[j][k] == i+1) 
                         {
                              tem = true; break;
                         }
               ...
               if (tem == true)
               {
                    choices[j][0] = 0; // Apaga a clausula
               }

               /* Apaga o valor da clausula */
               for (j = 0; j < m; j++)
               {
                    for (k = 0; k < 3; k++)
                    {
                         ...
                         if (choices[j][k] == -(i+1))
                         {
                              // Move os da frente para este lugar
                              for (z = k; z < 2; z++)
                              {
...
{
     int under = 0;
     /* Apply the one literal rule to the unit clauses until an empty clause is derived */
}
...
// Valores forcados atraves da Dominating Unit Clause
...
{     
     if (begin != end)
     {
          ...
          if (actual > best) return;
     }  
     if (begin == end) { 
          int soma = calc(c, end);
          if (soma == 407) showC(c,end);
...
int main()
{
     ...
     unitPropagation();
     int a;
     /* Preenche a outra forma de armazenar os dados */
     for (i = 0; i < m; i++)
     {     
          for (a = 0; a < 3; a++)
          {
               if (choices[i][a] == 0) break;
               int tmp = choices[i][a];
               if (tmp < 0)
     ...
     /*for (i = 0; i < n; i++)
     {
          // (1,1) literals escolhe o com menor peso
          if ((listaC[i][0] == 1) && (listaC[i][1] == 1))
          {
               if (friendship[lista[i][0][0]] < friendship[lista[i][1][0]]) 
                    forced[i] = -1;
               else
                    forced[i] = 1;
          }
     ...
     for (i = 0; i < n; i++)
     {
          /* Clausulas unitarias com valor i */
          int k = unitClauseC[i][1];

          /* Ocorrencias do literal -i */
          int k2 = listaC[i][0];
          if (k2 <= k) { forced[i] = 1; }
     }

     /* Almost Common Clause */
     showC(forced, n);
     //memcpy(c,forced, sizeof(unsigned char) * n);
     rec(c, 0, n);   
     printf("%d\n", best);
}
