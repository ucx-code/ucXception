...
int calcG2(unsigned char *c, int begin, int max)
{
register int i,a;
     unsigned char estado[2000];
     memset(estado, 0, sizeof(unsigned char) * m);
...
int calcG1(unsigned char *c, int begin, int max)
{
register int i,a;
     unsigned char estado[2000];
     memset(estado, 0, sizeof(unsigned char) * m);
...
{     
     ...
     for (i = 0; i < unitClauseC[begin][0]; i++)
     {
          n1 += friendship[unitClause[begin][0][i]];
     }
     int gp1 = calcG1(c, begin, end);
     int gn1 = calcG2(c,begin,end);
     if ((gp1 >= best) && (gn1 >= best)) return;
     if (gp1 >= best) 
     {
          c[begin] = 1;
          rec(c, begin+1, end);
