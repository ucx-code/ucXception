...
void shortestPath(int v, int len, int cap, int bound)
{
    ...
    cap -= distancias[v][17]; /* recalculo capacidade atual */
    
    if(v != 0) /* garantir precedencias */
    {
    ...
    if((aux + (bound - distancias[v][18]) / 2) >= best) /* aplicar bound */
        return;
