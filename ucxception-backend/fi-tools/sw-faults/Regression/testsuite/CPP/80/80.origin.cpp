          ...
          if (possuiIngredienteParcial == 1)
          {
               ...
               for (a = 0; a < 3; a++)
               {
                    ...
                    if (absChoice <= begin)
                    {
                         ...
                         {
                              satisfeito = 1; break;
                         } else if ((choices[i][a] > 0) && (c[absChoice] == 1)) {
                              satisfeito = 1; break;
                         }
                    }
               }
               if (satisfeito == 0) estado[i] = 0;
          }
...
int calcB(unsigned char *c, int begin, int max)
{
     ...
     for (i = 0; i < begin; i++)
     {
          if (c[i] == 0)
          {
               ...
               for (a = 0; a < listaC[i][1]; a++)
               {
                         if (estado[lista[i][1][a]] != 1) estado[lista[i][1][a]] = 0;
               }
          } else {
               for (a = 0; a < listaC[i][0]; a++)
               {
                          if (estado[lista[i][0][a]] != 1) estado[lista[i][0][a]] = 0;
               }
               for (a = 0; a < listaC[i][1]; a++)
               {
                         estado[lista[i][1][a]] = 1;
...
{     
     if (begin != end)
     {
          ...
          if (actual + minas[begin] > best) return;
     }  
     if (begin == end) { 
          int soma = calc(c, end);
...
int main()
{
     ...
     // Escolhe um melhor valor para o best
     srand(time(NULL));
     int a;
     for (a = 0; a < 2000; a++)
     {
     ...
     // Calcula a media de unitClauses
