    ...
    for(j=1; j<i; j++){
        ...
        while(fila[k][0]!=0){
            if(fila[k][0]<fila[ant][1] && fila[k][1]>fila[ant][1]){
                ...
                ant = k;
            }else if(fila[k][0]>fila[ant][1]){
                alt+=(fila[k][1]-fila[k][0]);
                ant=k;
                /*Perimetro: linhas horizontais interiores*/
        ...
        /*Calcular a area de x-1 ate x(aki)*/
        area=area+((begin[j][0]-begin[j-1][0])*(alt));
        
        /*Perimetro: linhas horizontais*/
        per+=2*(begin[j][0]-begin[j-1][0]);

        if(begin[j][0]<begin[j][2]){
            /*CANTO INFERIOR DIREITO*/
            k=0;
            while(fila[k][0]!=0 && fila[k][0]<begin[j][1]) k++;
            ...
            /*CANTO SUPERIOR ESQUERDO remoção*/
            k=0;
            while((fila[k][1]!=begin[j][1] && fila[k][0]!=begin[j][3]) && fila[k][0]!=0) k++;
