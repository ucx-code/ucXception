...
class Base_Campaign(object):
	...
	def _exec_parsers(self):
		...
		"""
		#Indicates whether you can run the parser 
		error = False
		...
		for parser in self.parsers:
			...
			for x in self._eval_lambda_funcs(parser_in, self):
				#Prevents to crash in extend function
				if x:	
					#Prevents that eval(x) cant be executed
					try:
						value = eval(x)
						if value != None:
							parser_in_exec.extend(value)
					except (ValueError, SyntaxError, AttributeError):
						error = True
						print("Unexpected error occur, campaign has no attribute called " + x)

			#If error true cant execute parser
			if not error:
				try:
	...
	def _extract_data(self):
		for probe_info in (self.pre_probes + self.post_probes):
			(probe_module, probe_target,  probe_in) = probe_info
			probe_key = (probe_module.__class__, probe_target,  probe_in)
			data_list = probe_module.extract_data()
			if data_list is not None:
				if probe_key not in self.gen_files:
					self.gen_files[probe_key] = [data_list,]
				else:
					self.gen_files[probe_key] = self.gen_files[probe_key] + [data_list,]
